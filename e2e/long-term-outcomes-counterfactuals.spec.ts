/**
 * E2E: long-term outcomes + counterfactual on Ergebnis pane (mobile widths).
 */
import { test, expect } from '@playwright/test'
import fs from 'node:fs'
import path from 'node:path'

const EVIDENCE = '.qa/evidence/long-term-outcomes-counterfactuals'
...
for (const width of [320, 375, 480] as const) {
  test(`long-term outcome vs counterfactual at ${width}px, DQ separated`, async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Ergebnis' }).click()
    await expect(page.getByTestId('immediate-effects-card')).toBeVisible()
    await expect(page.getByTestId('immediate-effects-card')).toContainText('Sofortwirkung')
    await expect(page.getByTestId('long-term-outcome-card')).toBeVisible({ timeout: 30_000 })
    await expect(page.getByTestId('outcome-summary-rows')).toContainText('Actual Cash')
    await expect(page.getByTestId('outcome-summary-rows')).toContainText('Median Cash')
    await expect(page.getByTestId('outcome-summary-rows')).toContainText('Perzentil')
    await expect(page.getByTestId('outcome-dq-separation')).toContainText('Decision Quality')
    ...
    await page.screenshot({
      path: path.join(EVIDENCE, `${width}-outcome.png`),
      fullPage: true,
    })
  })
}
