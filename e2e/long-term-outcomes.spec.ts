/**
 * E2E: long-term outcomes vs immediate effects, DQ separation on mobile.
 */
import { test, expect } from '@playwright/test'
import fs from 'node:fs'
import path from 'node:path'

const EVIDENCE = '.qa/evidence/long-term-outcomes'
...
async function enterLocalNordkern(page: import('@playwright/test').Page) {
  ...
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Wähle dein Unternehmen' })).toBeVisible({ timeout: 15_000 })
...
for (const width of [320, 375, 480] as const) {
  test(`outcome summary separated from DQ at ${width}px`, async ({ page }) => {
    ...
    await page.locator('textarea').fill(
      'Ziel: Marge und Resilienz. Annahme: Cash-Puffer. Alternative: nur Renegotiation.',
    )
    ...
    await expect(page.getByTestId('immediate-effects-card')).toBeVisible({ timeout: 20_000 })
    await expect(page.getByTestId('long-term-outcome-card')).toBeVisible()
    ...
    await page.screenshot({ path: path.join(EVIDENCE, `${width}-outcome.png`), fullPage: true })
  })
}
