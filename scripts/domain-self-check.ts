...
run = advanceTime(run, Math.max(1, (delayedDue ?? run.day) - run.day))
...
let soft = createRun('nexora-saas', 'self-check-deadline')
soft = advanceTime(soft, soft.deadlineDay - soft.day + 1)
assert(soft.ledger.some((event) => event.id === 'deadline_missed'), 'soft deadline consequence')
assert(soft.status === 'active', 'soft deadline does not force fail')
